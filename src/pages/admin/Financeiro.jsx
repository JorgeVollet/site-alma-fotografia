...
import { TrendingUp, TrendingDown, Wallet, Clock, ArrowUpRight, ArrowDownRight, Plus, Pencil, Trash2, X, Check, Filter, Loader2 } from 'lucide-react'
import { formatBRL } from '../../components/Money'
import { CATEGORIAS_FIN } from '../../data/crm'
...
export default function Financeiro() {
  ...
  const recarregar = useCallback(async () => {
    setCarregando(true)
    const [ls, contas] = await Promise.all([fetchLancamentos(), fetchContasReceber()])
    setLancamentos(ls)
    setAReceber(contas.filter((c) => c.status !== 'pago').reduce((s, c) => s + c.valor, 0))
    setCarregando(false)
  }, [])
  ...
  const cards = [
    { label: 'Entradas', valor: formatBRL(entradas), icon: TrendingUp, cor: 'text-clay-300', ring: 'ring-clay-400/20 bg-clay-400/5' },
    { label: 'Saídas', valor: formatBRL(saidas), icon: TrendingDown, cor: 'text-cream-100/70', ring: 'ring-cream-100/10 bg-cocoa-900' },
  ...
  const salvar = async (campos) => {
    if (editando.novo) await criarLancamento(campos)
    else await atualizarLancamento(editando.id, campos)
    setEditando(null)
    await recarregar()
  }
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-cream-100/60">Entradas e saídas reais. Reservas e contas recebidas entram sozinhas. "A receber" você gerencia em Contas.</p>
        </div>
        <button onClick={() => setEditando({ novo: true })} className="btn-light !py-2.5 text-xs"><Plus size={15} /> Novo lançamento</button>
      </div>
      ...
      {carregando ? (
        ...
        <p className="mt-4 rounded-2xl bg-cocoa-900 p-6 text-center text-sm text-cream-100/40 ring-1 ring-cream-100/10">Nenhum lançamento ainda. Reservas e contas recebidas aparecem aqui automaticamente.</p>
      ) : (
        <div className="mt-4 overflow-hidden rounded-2xl ring-1 ring-cream-100/10">
          <table className="w-full text-sm">
            <thead className="bg-cocoa-900 text-left text-xs uppercase tracking-wide text-cream-100/40">
              <tr><th className="px-5 py-3">Descrição</th><th className="hidden px-5 py-3 sm:table-cell">Categoria</th><th className="hidden px-5 py-3 md:table-cell">Data</th><th className="px-5 py-3 text-right">Valor</th><th className="px-5 py-3"></th></tr>
            </thead>
                  ...
                  <motion.tr key={l.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="group bg-cocoa-900/40 hover:bg-cocoa-900">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span>{l.descricao}{l.auto && <span className="ml-2 text-xs text-cream-100/30">(automático)</span>}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-4">
                      {!l.auto && (
                        <div className="flex gap-1 opacity-0 transition group-hover:opacity-100">
                          ...
                          <button onClick={() => remover(l.id)} className="grid h-7 w-7 place-items-center rounded-lg text-cream-100/40 hover:bg-terracotta-500/20 hover:text-terracotta-400"><Trash2 size={13} /></button>
                        </div>
                      )}
                    </td>
                  </motion.tr>
      ...
      <AnimatePresence>
        {editando && <EditorLancamento lanc={editando} clientes={clientes} onClose={() => setEditando(null)} onSalvar={salvar} />}
      </AnimatePresence>
    </div>
  )
}

function EditorLancamento({ lanc, clientes, onClose, onSalvar }) {
  ...
  const [data, setData] = useState(lanc.data || hojeISO())
  const [clienteId, setClienteId] = useState(lanc.clienteId || '')
  ...
  const salvar = () => {
    ...
    onSalvar({ tipo, descricao: descricao.trim(), valor: +valor, categoria, data, clienteId: clienteId || null })
  }
      ...
      <motion.div initial={{ scale: 0.96, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.96, opacity: 0 }} transition={{ type: 'spring', stiffness: 300, damping: 26 }} onClick={(e) => e.stopPropagation()} className="max-h-[88vh] w-full max-w-md overflow-y-auto rounded-3xl bg-cocoa-900 p-7 ring-1 ring-cream-100/10">
        ...
        <div className="mt-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <label className="block"><span className="text-sm text-cream-100/80">Cliente (opcional)</span><select className={inp} value={clienteId} onChange={(e) => setClienteId(e.target.value)}><option value="">—</option>{clientes.map((c) => <option key={c.id} value={c.id}>{c.nome}</option>)}</select></label>
          </div>
        </div>
        <button onClick={salvar} disabled={!descricao.trim() || !valor} className="btn-light mt-7 w-full disabled:opacity-40"><Check size={16} /> {lanc.novo ? 'Adicionar' : 'Salvar'}</button>
      </motion.div>
    </motion.div>
  )
}
